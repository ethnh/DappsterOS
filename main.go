...
var (
	...
	//go:embed api/dappsteros/openapi.yaml
	_docYAML string

	//go:embed build/sysroot/etc/dappsteros/dappsteros.conf.sample
	_confSample string
...
func main() {
	...
	urlFilePath := filepath.Join(config.CommonInfo.RuntimePath, "dappsteros.url")
	...
	if supported, err := daemon.SdNotify(false, daemon.SdNotifyReady); err != nil {
		logger.Error("Failed to notify systemd that dappsteros main service is ready", zap.Any("error", err))
	} else if supported {
		logger.Info("Notified systemd that dappsteros main service is ready")
	} else {
	...
	logger.Info("Dappster main service is listening...", zap.Any("address", listener.Addr().String()))
