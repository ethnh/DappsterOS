...
readonly DAPPSTER_SERVICES=(
    "dappsteros.service"
    "devmon@devmon.service"
)

readonly CASA_EXEC=dappsteros
readonly CASA_CONF=/etc/dappsteros/dappsteros.conf
readonly CASA_URL=/var/run/dappsteros/dappsteros.url
readonly CASA_SERVICE_USR=/usr/lib/systemd/system/dappsteros.service
readonly CASA_SERVICE_LIB=/lib/systemd/system/dappsteros.service
...
Detecting_DappsterOS() {
    if [[ ! -x "$(command -v ${CASA_EXEC})" ]]; then
        Show 2 "DappsterOS is not detected, exit the script."
        exit 1
    else
        Show 0 "This script will delete the containers you no longer use, and the DappsterOS configuration files."
    fi
}
...
Uninstall_Casaos() {
    ...
    for SERVICE in "${DAPPSTER_SERVICES[@]}"; do
    ...
    # Old Casa Files
    if [[ -d ${CASA_PATH} ]]; then
        rm -rvf ${CASA_PATH} || Show 1 "Failed to delete legacy DappsterOS files."
    fi
    ...
    rm -rvf /var/lib/dappsteros/app_category.json
    rm -rvf /var/lib/dappsteros/app_list.json
    rm -rvf /var/lib/dappsteros/docker_root
}
...
if [[ ${UNINSTALL_ALL_CONTAINER} == true ]]; then
    ...
    while true; do
        echo -n -e "         ${aCOLOUR[4]}Do you want delete all AppData of DappsterOS? Y/n :${COLOUR_RESET}"
        read -r input
        case $input in
        [yY][eE][sS] | [yY])
