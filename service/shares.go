 ...
 * @FilePath: /DappsterOS/service/shares.go
 * @Description:
...
func (s *sharesStruct) UpdateConfigFile() {
	...
	for _, share := range shares {
		...
		configStr += `
[` + dirName + `]
comment = DappsterOS share ` + dirName + `
public = Yes
path = ` + share.Path + `
browseable = Yes
read only = No
...
func (s *sharesStruct) InitSambaConfig() {
	if file.Exists("/etc/samba/smb.conf") {
		...
		if strings.Contains(str, "# Copyright (c) 2021-2022 DappsterOS Inc. All rights reserved.") {
			return
		}
		file.MoveFile("/etc/samba/smb.conf", "/etc/samba/smb.conf.bak")
		smbConf := ""
		smbConf += `# Copyright (c) 2021-2022 DappsterOS Inc. All rights reserved.
#
#
#                          ______     _______
#                        (  __  \   (  ___  )
...
#
#
# IMPORTANT: DappsterOS will not provide technical support for any issues
