...
import (
	...
	"github.com/dappster-io/DappsterOS/pkg/utils/ip_helper"
...
func (c *systemService) GenreateSystemEntry() {
	modelsPath := "/var/lib/dappsteros/www/modules"
...
func (c *systemService) GetDirPath(path string) ([]model.Path, error) {
	if path == "/DATA" {
		...
		if sysType == "windows" {
			path = "C:\\DappsterOS\\DATA"
		}
		if sysType == "darwin" {
			path = "./DappsterOS/DATA"
		}
...
func (s *systemService) UpdateSystemVersion(version string) {
	...
	// go command2.OnlyExec("curl -fsSL https://raw.githubusercontent.com/LinkLeong/dappsteros-alpha/main/update.sh | bash")
	if len(config.ServerInfo.UpdateUrl) > 0 {
		go command.OnlyExec("curl -fsSL " + config.ServerInfo.UpdateUrl + " | bash")
	} else {
		osRelease, _ := file.ReadOSRelease()
		go command.OnlyExec("curl -fsSL https://get.dappsteros.io/update?t=" + osRelease["MANUFACTURER"] + " | bash")
	}
...
func (s *systemService) GetDappsterOSLogs(lineNumber int) string {
